package main

import (
	"crypto/rsa"
	"crypto/x509"
	"encoding/base64"
	"flag"
	"fmt"
	"io/ioutil"
	"math/big"
	"time"
)
const keyForSign = ""

func main() {
	text := flag.String("text", "", "The plaintext to be encrypt/decrypt")
	mode := flag.Int("mode", 0, "Mode: 0 -> encrypt(default), 1 -> decrypt")
	flag.Parse()


	decodeString, _ := base64.StdEncoding.DecodeString(keyForSign)
	privateKey, err := x509.ParsePKCS1PrivateKey(decodeString)
	if err != nil {
		fmt.Println(err)
	}
	var result string
	before := time.Now()
	switch *mode {
	case 0:
		result = encryptRsaForSign(privateKey.PublicKey, *text)
	case 1:
		result = decryptRsaForSign(*privateKey, *text)
	}
	after := time.Now()
	fmt.Println(result)
	err = ioutil.WriteFile("./result", []byte(result), 0644)
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println("cost", (after.UnixNano() - before.UnixNano()) / int64(time.Millisecond), "ms")
}

func decryptRsaForSign(privateKey rsa.PrivateKey, text string) string {
	decodeString, err := base64.StdEncoding.DecodeString(text)
	if err != nil {
		fmt.Println(err)
	}
	ciphertext := new(big.Int).SetBytes(decodeString)
	plaintext := ciphertext.Exp(ciphertext, privateKey.D, privateKey.N).Bytes()
	return base64.StdEncoding.EncodeToString(plaintext)
}

func encryptRsaForSign(publicKey rsa.PublicKey, text string) string {
	decodeString, err := base64.StdEncoding.DecodeString(text)
	if err != nil {
		fmt.Println(err)
	}
	plaintext := new(big.Int).SetBytes(decodeString)
	ciphertext := plaintext.Exp(plaintext, big.NewInt(int64(publicKey.E)), publicKey.N).Bytes()
	return base64.StdEncoding.EncodeToString(ciphertext)
}
